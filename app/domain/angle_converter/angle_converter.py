...
class AngleConverter:
    ...
    def generate_correct_trajectory(self, time_unit: float = 0.7) -> CorrectTrajectory:
        ...
        for _, row in change_df.iterrows():
            ...
            rssi1 = 0
            rssi2 = 0
            correct_trajectory.append([x, y, step, direction, angle_change, rssi1, rssi2])

        return CorrectTrajectory(trajectory=correct_trajectory)

    def __calculate_cumulative_angle(self, gyro_df: pd.DataFrame, time_unit: float) -> pd.DataFrame:
        sample_freq = 100
        window_gayo = 120
        ...
        gyro_df["angle_x"] = gyro_df["angle"].rolling(window=window_gayo, center=True).mean()
                ...
                lambda df: pd.Series(
                    {
                        ...
                        "angle_x": np.trapezoid(df["angle_x"], df["t"])
